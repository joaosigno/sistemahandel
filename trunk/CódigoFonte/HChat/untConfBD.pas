...
uses untDM, untDeclaracoes, untAutenticacao, untPrincipal;
...
//p  dm.con.Params.insert(1,'Database=' + inifile.readstring('BD','server','') + ':' + inifile.readstring('BD','path',''));
  ...
  try
    ...
    if dm.scHC.Connected = true then
    begin
      frmPrincipal.OnShow(Sender);
{      frmPrincipal.IdTCPClient1Disconnected(IdTCPClient1);

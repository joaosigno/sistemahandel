...
type
  TfrmManuGrades = class(TfrmManutencao)
    ...
    procedure cdsColGradeAfterCancel(DataSet: TDataSet);
...
begin
  if (tag=5) OR (tag = 3) then //verifica se a tabela está em modo de edição e se tiver pergunta se o cliente quer salvar, nao salvar ou cancelar
  begin
    case MessageDlg('ATENÇÃO: Tem dados não salvos na tabela!' + #13 + 'Deseja salvá-los?', mtConfirmation, mbYesNoCancel, 0) of
      mryes:cdsGDProd.Post;
      mrno:cdsGDProd.Cancel;
      mrcancel:abort;
    end;
  end;
  DAOLinha.cancelar;
...
begin
//  inherited;
  ManuDAO.SQL.executaSQlPorEmp(dm.cdsAux,'*','produ',' and cdgrad='+
            IntToStr(cdsGDProdcdgrad.AsInteger));
  if dm.cdsAux.RecordCount = 0 then
  begin
    if MessageDlg('Deseja Exluir essa Grade?', mtConfirmation ,[mbYes,mbNo], 0) = mryes then
    begin
       cdsLinhGrade.First;
       while not cdsLinhGrade.Eof do
       begin
  ...
  begin
      f.Mensagem(false,'Não Foi possível Excluir, Existem Produtos Relacionados a Essa Grade:'+
            cdsGDProddescri.AsString);
  end;
...
procedure TfrmManuGrades.cdsColGradeAfterCancel(DataSet: TDataSet);
begin
  cdsColGrade.Tag := 0;
end;

procedure TfrmManuGrades.cdsLinhGradeAfterCancel(DataSet: TDataSet);
begin
   cdsLinhGrade.Tag := 0;
end;

procedure TfrmManuGrades.cdsGDProdAfterEdit(DataSet: TDataSet);
begin
    tag:=5;
end;

...
begin
  ...
  8:
   begin
        if (rb1.Checked) and (edtpesquisa.text <> '') then
          SQL.executaSQlPorEmp(cdsProcuraRapida,'nrcart,nmcart',' crfid ',
                ' and nrcart='+ QuotedStr(edtPesquisa.Text)+' order by nrcart ');
  ...
  10:
   begin
        if (rb1.Checked) and (edtpesquisa.text <> '') then
          SQL.executaSQlPorEmp(cdsProcuraRapida,'cdvend,nome',' vende ',
                ' and cdvend='+ QuotedStr(edtPesquisa.Text)+' order by cdvend ');
  ...
  11:
   begin
        if (rb1.Checked) and (edtpesquisa.text <> '') then
          SQL.executaSQlPorEmp(cdsProcuraRapida,'cdmarc,descri',' mcpro ',
                ' and cdmarc='+ QuotedStr(edtPesquisa.Text)+' order by cdmarc ');
  ...
  15:
   begin
        if (rb1.Checked) and (edtpesquisa.text <> '') then
          SQL.executaSQlPorEmp(cdsProcuraRapida,'cdcont,nrcont,agenci,descri',' ctban ',
                '  and nrcont like '+ QuotedStr('%'+edtPesquisa.Text+'%')+' order by nrcont ');

       if (rb2.Checked) and (edtpesquisa.text <> '') then
          SQL.executaSQlPorEmp(cdsProcuraRapida,'cdcont,nrcont,agenci,descri',' ctban ',
               '  and agenci like '+ QuotedStr('%'+edtPesquisa.Text+'%')+' order by agenci ');
...
begin
   if (rb1.Checked) and (pesq <> 'ctban')  then
  begin
...
begin
    ...
    case c of
      ...
      15: visible(true,'Conta',true,'Agencia',true,'Descrição',false,'','cdcont,nrcont,agenci,descri','ctban','order by cdcont');
...
begin
    case c of
      ...
      begin
       ...
       8:
      begin
          dm.cdsCartFid.Locate('NRCART', VarArrayOf([cdsProcuraRapida.Fields[0].AsString]), [loPartialKey]);
      end;
      9:
      begin
      ...
      10:
      begin
          dm.cdsVende.Locate('CDVEND', VarArrayOf([IntToStr(cdsProcuraRapida.Fields[0].AsInteger)]), [loPartialKey]);
      end;
      11:
      begin
          dm.cdsMcPro.Locate('CDMARC', VarArrayOf([IntToStr(cdsProcuraRapida.Fields[0].AsInteger)]), [loPartialKey]);
      end;
      12:
      begin
      ...
      15:
      begin
          dm.cdsCTBan.Locate('CDCONT', VarArrayOf([IntToStr(cdsProcuraRapida.Fields[0].AsInteger)]), [loPartialKey]);
      end;
    end;
    sql.Destroy;
end;
...
begin
    case c of
      ...
      begin
          ...
          gridPesquisa.Columns.Items[1].Title.Caption := 'Dt.Cadastro';
      ...
      begin
          gridPesquisa.Columns.Items[0].Title.Caption := 'Código';
          gridPesquisa.Columns.Items[0].Width := 80;
          gridPesquisa.Columns.Items[1].Title.Caption := 'Conta';
          gridPesquisa.Columns.Items[1].Width := 80;
          gridPesquisa.Columns.Items[2].Title.Caption := 'Agencia';
          gridPesquisa.Columns.Items[2].Width := 80;
          gridPesquisa.Columns.Items[3].Title.Caption := 'Descrição';
          gridPesquisa.Columns.Items[3].Width := 300;
      end;
    end;  
